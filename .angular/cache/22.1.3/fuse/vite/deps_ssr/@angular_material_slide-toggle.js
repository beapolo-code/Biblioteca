...
var MatSlideToggle = class MatSlideToggle {
	...
	ngAfterContentInit() {
		this._focusMonitor.monitor(this._elementRef, true).subscribe((focusOrigin) => {
			if (focusOrigin === "keyboard" || focusOrigin === "program") {
				this._focused = true;
				this._changeDetectorRef.markForCheck();
			} else if (!focusOrigin) Promise.resolve().then(() => {
				this._focused = false;
				this._onTouched();
				this._changeDetectorRef.markForCheck();
			});
	...
	ngOnChanges(changes) {
		if (changes["required"]) this._validatorOnChange();
	}
	ngOnDestroy() {
	...
	static ɵcmp = /* @__PURE__ */ ɵɵdefineComponent({
		...
		consts: [
			...
			[1, "mat-mdc-slide-toggle-touch-target"],
			[1, "mdc-switch__track"],
...
var MatSlideToggleModule = class MatSlideToggleModule {
	...
	static ɵinj = /* @__PURE__ */ ɵɵdefineInjector({ imports: [MatSlideToggle, BidiModule] });
};
(() => {
